...
class FTPServer(object):
    ...
    def authenticate(self,username,password):
        ...
        if username in self.accounts:
            ...
            if md5_password == _password:
                ...
                self.user_obj['user_home']= os.path.join(settings.USER_HOME_DIR,username) #/user_home/alex
                #set user user_home directory
                self.user_current_dir = self.user_obj['user_home']
                return True
            else:
    ...
    def _get(self,data):
        ...
        #full_path = os.path.join(self.user_obj['user_home'],filename)
    ...
    def _re_get(self,data):
        ...
        full_path = os.path.join(self.user_obj['user_home'],abs_filename.strip("\\"))
        print("reget fullpath", full_path)
        print("user user_home",self.user_obj['user_home'])
    ...
    def _cd(self,data):
        ...
        """
        #/user_home/alex/FuckHomework/  cfd
        target_dir = data.get('target_dir')
        full_path = os.path.abspath(os.path.join(self.user_current_dir,target_dir) ) #abspath是为了解决../..的问题
        print("full path:",full_path)
        if os.path.isdir(full_path):

            if full_path.startswith(self.user_obj['user_home']):#has permission
                self.user_current_dir = full_path
                relative_current_dir = self.user_current_dir.replace(self.user_obj['user_home'], '')
