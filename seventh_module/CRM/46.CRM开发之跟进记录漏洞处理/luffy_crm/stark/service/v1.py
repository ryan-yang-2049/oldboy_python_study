...
class StarkHandler(object):
	...
	def add_view(self, request, *args, **kwargs):
		...
		if form.is_valid():
			httpresponse = self.form_database_save(request, form, False, *args, **kwargs)
			# 在数据库中保存成功后,跳转回列表页面（携带原来的参数）
			return httpresponse or redirect(self.reverse_list_url(*args, **kwargs))
		return render(request, 'stark/change.html', {"form": form})
	...
	def change_view(self, request, pk, *args, **kwargs):
		...
		if form.is_valid():
			httpresponse = self.form_database_save(request, form, True, *args, **kwargs)
			# 在数据库中保存成功后,跳转回列表页面（携带原来的参数）
			return httpresponse or redirect(self.reverse_list_url(*args, **kwargs))
		return render(request, 'stark/change.html', {"form": form})
