...
class ConsultRecordHandler(StarkHandler):
	...
	def get_change_object(self,request, pk, *args, **kwargs):
		...
		return models.ConsultRecord.objects.filter(pk=pk,customer_id=customer_id,customer__consultant_id=current_user_id).first()

	def delete_object(self, request, pk, *args, **kwargs):
		current_user_id = request.session['user_info']['id']
		customer_id = kwargs.get('customer_id')

		record_queryset = models.ConsultRecord.objects.filter(pk=pk,customer_id=customer_id,customer__consultant_id=current_user_id)
