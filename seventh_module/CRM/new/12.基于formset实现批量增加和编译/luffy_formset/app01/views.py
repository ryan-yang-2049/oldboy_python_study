...
def multi_add(request):
	...
	:return:
	"""
	formset_class = formset_factory(MultiPermissionForm, extra=5)   # extra 表示批量操作几个form表单
...
def multi_edit(request):
	formset_class = formset_factory(MultiUpdatePermissionForm, extra=0) # 默认 extra=1
	if request.method == 'GET':
		formset = formset_class(
