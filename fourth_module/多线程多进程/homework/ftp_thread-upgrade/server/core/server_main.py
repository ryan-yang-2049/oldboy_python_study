...
class MySocket(object):
	...
	def connect(self):
		...
		while True:
			...
			self.queue.put(conn)
			try:
				t = self.pool.submit(self.login_info_auth,self.queue.get())
			except queue.Empty:
				continue
	...
	def login_info_auth(self,conn):
		print("开始认证，conn:",conn)
		while True:
			try:
				print("1.开始认证用户信息")
				res = conn.recv(1024)
				self.user_dict = json.loads((res).decode(self.coding))
				print("2.用户登陆字典：",self.user_dict)
				...
				if return_res and hasattr(self, 'run'):
					print("thread name is : %s" % currentThread().getName())
					conn.send(return_res_bytes)
					getattr(self, 'run')(conn)
				else:
					conn.send(return_res_bytes)
					print("认证成功，thread name is : %s" % currentThread().getName())
					continue
	...
	def run(self,conn):
		...
		while True:
			try:
				print("打印run 方法里面的conn:", conn)
				# print("self.inputs",self.inputs)
				cmd_res = conn.recv(self.MAX_PACKET_SIZE)
				if not cmd_res:
					print("没有接收到消息",cmd_res)
					break
				print("接收用户命令参数:%s"%cmd_res)
	...
	def put_file(self,cmds,conn):
		...
		# filename = "%s%s%s" % (self.dir_li, self.PATH_SYMBOL, cmds[1])
		print("服务端存文件",filename)
	...
	def remove_dir(self,cmds,conn):
		...
		if len(cmds) == 2:
			switch_dir = cmds[1]
			if switch_dir == '..':
				print("1，查看当前目录列表",self.dir_li)
				print("2，查看用户家目录",self.absolute_dir)
